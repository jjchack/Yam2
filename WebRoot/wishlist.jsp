			   ...
			   <c:when test="${!empty sessionScope.wishlist}"> 
			   		...
			   		<c:forEach items="${sessionScope.wishlist }" var="list" varStatus="varSta">
						<tr>
				  			...
				  			<td><a href="GetSingleById?goodsid=${list.goodsid }">点击查看详情</a></td>
				  			<td>${list.goodsnum }</td>
				  			<td>${list.singleprice }</td>
				  			<td>${list.totalprices }</td>
				  			<td><div id="delete"><a href="${pageContext.request.contextPath }/DeleteShopById?shoppingid=${list.id }">从购物车中删除</a></div></td>
			  			</tr>
					</c:forEach> 
			   </c:when>
			   <c:otherwise> 
			   		
			   		<h6>您的购物车中暂无商品，<a href="GetDiffGoods?goodstype=铁棍山药" >赶紧去添加吧！！！</a></h6>
